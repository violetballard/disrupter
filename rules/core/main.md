...
A Panic Check requires rolling over current Stress with a d20 (See [Rule 3](#rule3)) or the rolled effect occurs from the appropriate [Panic Table][panic] (Individual or Platoon).
...
- Make a Stand Panic Check for each Stand within Close Range of a human death, including the stand with the dead soldier
...
*[streamline] Make sure every Stand with a human contains at least 1 Officer. Officers act on their own volition, so this eliminates the entire command aspect of the game, making the game play like a more traditional, direct control wargame. If a Stand consists entirely of Bots it still needs to be led by another Stand with an Officer. This eliminates the need for written orders (see [Rule 9 - Giving Orders](#rule9.orders)). If you combine this with the [streamline] single Wound rule (See [Rule 6](#rule6)), your [Platoon Card][platoon] only serves as a reference that you do not have to keep up to date with Current Orders and Wounds.*

[platoon]: /platoons/platoon_card.md

### <a name="rule9.classes"></a>**Classes**
There are three classes of units that enable the chain of command. All units are equipped with [Weapons][weapons] chosen by each Platoon during the Intelligence Briefing (See Rule #). Wounds (W) and Movement (M) are determined by equipped [Armor][armor] in the case of humans and model specifications for [Bots][bots].
...
Officers are able to give Orders to lower ranking officers (CO to NCO) and soldiers. Orders may be given over the radio. There is no range to radio, and you may assume that every unit has access to a helmet radio. Orders are short written statements recorded on a [Platoon Card][platoon]. Units follow their orders without deviation with small exceptions for Taking Cover (See Rule #), avoiding Fire (See Rule #), and returning with Reaction Fire (See Rule #) for self-defense.
...
Officers and anyone they lead are able to freely use Movement Actions and Battlefield Actions (See Rule #). An Officer is able to lead any units that are within Close Range (See [Rule 5 - Range Bands](#rule5.range)). Bots are unable to follow orders and can only be led. Bots can only retreat and take cover and conduct Reaction Fire (See Rule #) for self-defense if they are not within Close Range of an officer.

## **Game Structure**

### <a name="rule10"></a>**Rule 10 - Intelligence Briefing**
The Intelligence Briefing functions much like a session zero in a ttrpg. The Intelligence Briefing may take place on the table right before a game, and it can take weeks of negotiation.
...
Once the structure of the game has been decided, COs are able to:
...
- Create up to four code word commands for orders for each NCO (See [Rule 9 - Orders](#rule9.orders))
    - Halt, Rally to specified Rally Point, Retreat, and Resume are also default Code Words in addition to CO created orders
...
Every turn of the game consists of an Initiative Phase, and an Action Phase. During the Initiative Phase turn order is decided. During the Action Phase Stands may choose to take a Movement Action followed by a Battlefield Action. Actions must be performed in that order during a turn.

#### <a name="rule11.init"></a>**Rule 11 - Intiative Phase**
1. Each Platoon gains 1 Stress
2. Both COs make an Instinct Check
    - Successful NCO goes first
    - If both are Successful the highest roll goes first
...
#### <a name="rule11.action"></a>**Rule 11 - Action Phase**
Actions take place in the order determined during the Initiative Phase (see [Rule 11](#rule11)). Each CO and the Squad they command must take their actions at the same time during each portion of the Action Phase.
...
COs can choose to relieve 1 Stress, replacing both their Movement and Battlefield Actions for the turn.
...
Line of Sight (LOS) is determined by Stand bases (see [Rule 4 - Basing](#rule4.basing)).

If you can draw a straight line between any portion of two Stands without any Obstructions, there is LOS.

Friendly Stands are ignored as Obstructions.
...
If a whole stand is fired on, only one Armor Save (See [Rule 7](#rule7)) is made for all of the units with the same Armor Stat (See [Rule 6](#rule6)).

If multiple units with the same Instinct Stat (See [Rule 6](#rule6)) are firing at the same unit/stand only one Instinct Check (See [Rule 7](#rule7)) is made.

The number of Wounds inflicted with a Successful attack is listed for each range for each [Weapon][weapons].

When Firing on a Stand, the total number of Wounds landed is divided evenly amongst units on the Stand. Randomly assign Wounds if they don’t split evenly.

*[streamline] All weapons hit for 1 Wound. This way, you only need the Max Stat (See [Weapons][weapon])to determine if a Weapons hits. If you combine this with the [streamline] rule for all Units to have 1 Wound (See [Rule 6](#rule6)), you have a very quickly moving game. If you then use either [streamline] variant for single Checks or Saves to resolve Battlefield Actions, your game has barely any tracking or in-the-moment rules. You can see how much damage your Platoon can take just by looking at the number of Units on the board. This combination of rules is what makes the [streamline] version of Disrupter special.*

#### <a name="rule16.command"></a>***Rule 16 - Commanding***
With one Battlefield Action an Officer can give one set of Orders to a single NCO or Squad. A CO can give Code Word Orders to as many NCOs as they want with a single Battlefield Action.

Giving Orders is automatically successful.

CO orders to NCOs with code words are executed immediately. New CO to NCO orders are executed starting the next turn. All NCO orders to squads occur immediately.

Officers can attempt Rally Stands within Close Range with a Successful Instinct Check. Rallying troops brings Routed Stands (See [Rule 8](#rule8) and [Panic][panic]) under the Command of the Rallying Officer.

*[streamline] Rally Checks are automatically Successful.*
