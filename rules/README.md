...
You can build your own terrain if that's your thing, but we highly suggest using things you already have or using play surfaces from other games and from toys. We want you play, not worry about your crafting skills. For a city environment, we suggest using the tiles and buildings from a Dropzone Commander [City][city]/[Ruinscape][ruin]. For a jungle environment, we recommend a [Children's Playset][playset]. There are plenty of digital maps available that you can adapt if you are wanting to play using a VTT.

[city]: https://ttcombat.com/collections/dropzone-commander/products/cityscape
[ruin]: https://ttcombat.com/collections/dropzone-commander/products/ruinscape?variant=31743811158064
...
For traditional basing, miniatures for Infantry should be based four to a Stand (See [Size of Force][size]). You need some way to remove or mark (small rubber bands are recommended) Units when they need to be removed from the Board if you want to use traditional basing.
...
You do not need to play on a hex grid (see (./#rule2)) and [Rule 4 - Basing](./#rule4.basing)); it is possible to use ruler measurement as well. Measure from the same part of the stand to the same part of the stand (center to center, front to front, etc.) using the following conversion factor. You can use this conversion factor to help you scale up or down your miniatures as well, depending on what you have available to you.
...
_\[streamline] All Units only have 1 Wound, regardless of_ [_Equipment_](/equipment)_. This makes the game more lethal, and it means that you no longer need to track Wounds on your_ [_Platoon Card_][card]_._
...
* The specific rules being used for the game including the [Game Mode](/game_modes)
* The size and composition of forces, documented on [Platoon Cards][card]
...
Vehicles have different LOS rules than Infantry (See [Vehicles]([vehicle_rules])

[vehicles]: /equipment/vehicles.md
[vehicle_rules]: /rules/vehicles.md
...
You can attempt to Spot an enemy within LOS by making an Instinct Check in order to draw a target out of Cover, paint a target for a missile, or identify a target for a sniper. You can track spotting on the [Platoon Card][card] or using markers on the board.
